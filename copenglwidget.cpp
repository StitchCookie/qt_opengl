...
void COpenGlWidget::initializeGL()
{
    ...
    if(!shaderProgramLighting.isLinked()) qDebug()<<"ERR:"<<shaderProgramLighting.log();
    m_mash = processMesh();
}
...
{
    vector<Vertex> _vertices(36);  // 初始化顶点属性数组大小为36个
    vector<unsigned int> _indices;
    vector<Texture> _textures;
    // _vertices.reserve(36);
    // _vertices.resize(36);
    // 一定不能要用内部存储的第一个元素的地址。 而不能vector的首地址
    ...
    Texture tex;
    tex.id=m_Texture->textureId();  //  一个纹理对象通常会有一个唯一的标识符（ID），用于在GPU端引用和操作这个纹理。
